    ...
    while(1) {  // main accept() loop
        ...
        if (!fork()) { // this is the child process
			...
			if (strncmp(buf, "GET", 3) == 0) {
                    ...
                    //2016-1-10 17:30:52,192.168.1.72,414;388-2
                    ...
                    do {
                        ...
                        while ((buf[bufindex] != '\0') && (buf[bufindex] != ';')) {
                            tmpbuf[tmpbufindex++] = buf[bufindex++];
                        }
...
//                        }
                        ...
                        sprintf(buffer, "%i-%i-%i %i:%i:%i,%s,%s-%d\n\0", current->tm_year+1900, current->tm_mon+1, current->tm_mday, current->tm_hour, current->tm_min, current->tm_sec, s, tmpbuf, id);
                        ...
                        int buf_int = atoi(tmpbuf);
                        if ((buf_int < 1360) && (buf_int > -1600)) {
struct timeval tv;
...
start += (tv.tv_sec * 1000);
                            sprintf(buffer, "UPDATE temp_rt SET value=%s, timestamp=\"%i-%i-%i %i:%i:%i\" WHERE tempid = %d\0", tmpbuf, current->tm_year+1900, current->tm_mon+1, current->tm_mday, current->tm_hour, current->tm_min, current->tm_sec, id);
                            printf("%d-%s\n",id,buffer);
