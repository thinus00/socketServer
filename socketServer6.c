    ...
    while(1) {  // main accept() loop
		...
		while (pch != NULL) {
			...
			printf("%i\n",values[valueCnt-1]);
                ...
                avg = (int)avg/valueCnt;
                printf("Max: %i\n", maxValue);
                printf("Min: %i\n", minValue);
                printf("Avg: %i\n", avg);
		valueDiff = maxValue - minValue;
		printf("valueDiff: %i\n", valueDiff);
		printf("p: %f\n", ((float)valueDiff/(float)minValue)*100);

                avg = 0;
                avgCount = 0;
                int ic = 0;
		for(ic = 0; ic < valueCnt; ic++) {
                    if ((values[ic] != maxValue) && (values[ic] != minValue)) {
                        avg = avg + values[ic];
                        avgCount++;
                    }
                }
                printf("avg: %i avgCount: %i\n", avg, avgCount);
                avg = (int)avg/avgCount;
                printf("Avg*: %i\n", avg);
		int diffPrevAvg = prevAvg - avg;
		printf("diffPrevAvg: %i\n", diffPrevAvg);
                printf("p2: %f\n", ((float)diffPrevAvg/(float)avg)*100);
